...
namespace trucompute_collapse_v8 {

// Native field ontology.
// BOTH is the resting condition of every TruCompute field node.
// NEG/POS are transient observation collapses.
// NEITHER belongs only to the machine switch boundary.
enum class NativeFieldState : uint8_t {
    BOTH
};

enum class Collapse : int8_t {
    NEG = -1,
    POS = +1
};
...
public:
    ...
    const char* native_type() const override { return "TRUCOMPUTE_BOTH_FIELD_NODE"; }
    uint32_t position() const { return position_; }

    // The node is never rewritten away from BOTH.
    constexpr NativeFieldState native_state() const { return NativeFieldState::BOTH; }

    // "Overwrite without overwrite": different conditioned overlays coexist.
    // Only two incompatible writes under the exact same condition contradict.
    void imprint(uint16_t page,uint16_t symbol){
        for(const auto& x:overlays_){
            if(x.page!=page) continue;
            if(x.symbol==symbol) return; // idempotent
    ...
    uint16_t symbol_for(uint16_t page) const {
        ...
        throw std::runtime_error("selected page has no conditioned overlay");
    }

    // Observation collapse is transient and pure: no node mutation.
    Collapse observe(const Selector& s) const {
        return symbol_for(s.page)==s.character ? Collapse::POS : Collapse::NEG;
    }
};
...
class Field {
    ...
    std::vector<std::unique_ptr<CharacterActivatorNode>> chars_;
...
public:
    ...
    QueryFrame observe(uint16_t page,uint16_t character) const {
        ...
        frame.collapses.reserve(data_.size());
        for(const auto& n:data_)
            frame.collapses.push_back(n->observe(s)); // every data node collapses +/-1
