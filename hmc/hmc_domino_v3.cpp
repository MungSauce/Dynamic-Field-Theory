...
static void encode(const std::string&src,const std::string&dst,uint8_t minmatch){
    ...
    out.write("HC31",4);out.put(1);out.put(char(minmatch));put_u64(out,n);
...
static void decode(const std::string&src,const std::string&dst){
    ...
    char magic[4];in.read(magic,4);if(in.gcount()!=4||std::string(magic,4)!="HC31")throw std::runtime_error("magic");
...
int main(int argc,char**argv){
    try{
        if(argc<4||argc>5){std::cerr<<"usage: hmc_domino_v3 e input output.hcd [minmatch=8] | d input.hcd output\n";return 2;}
