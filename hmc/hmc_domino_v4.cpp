...
struct MEntry{uint64_t key=0;std::array<uint32_t,4> pos{};uint8_t count=0;};
...
class Model{
    static constexpr uint32_t HSZ=1u<<20, HMASK=HSZ-1, MSZ=1u<<22, MMASK=MSZ-1, MAXG=4095;
    std::array<BinCtx,256> global_{};
    std::vector<BinCtx> o1_;
    std::vector<HEntry> o2_,o3_,o4_,o5_;
    ...
    uint64_t pos_=0;
    uint8_t prev1_=0,prev2_=0,prev3_=0,prev4_=0,prev5_=0;
    bool have1_=false,have2_=false,have3_=false,have4_=false,have5_=false;
    int match_pred_=-1;
    std::array<uint32_t,4> match_sources_{};
    ...
    void prepare_special(){
        ...
        match_pred_=-1;match_count_=0;
        if(hist_.size()>=8){
            uint64_t key=context8_key();uint32_t idx=uint32_t(key)&MMASK;
            const MEntry&e=match_[idx];
            if(e.key==key){
                for(uint8_t j=0;j<e.count && j<4;j++){
                    if(e.pos[j]<hist_.size())match_sources_[match_count_++]=e.pos[j];
                }
                if(match_count_)match_pred_=hist_[match_sources_[0]];
            }
        }
    }
public:
    Model():o1_(256*256),o2_(HSZ),o3_(HSZ),o4_(HSZ),o5_(HSZ),match_(MSZ),gapfreq_(256*(MAXG+1),0){
        last_.fill(-1);
        hist_.reserve(1<<20);
    }
    void reserve(size_t n){hist_.reserve(n);}
    void start_byte(){prepare_special();}
    uint8_t match_count() const{return match_count_;}
    uint32_t match_source(uint8_t i) const{if(i>=match_count_)throw std::runtime_error("match candidate");return match_sources_[i];}
    ...
    void probs(uint16_t node,uint32_t &f0,uint32_t &f1){
        ...
        if(have4_){
            uint64_t k=(uint64_t(prev4_)<<40)|(uint64_t(prev3_)<<32)|(uint64_t(prev2_)<<24)|(uint64_t(prev1_)<<16)|node;
            add(get(o4_,k).c,10);
        }
        if(have5_){
            uint64_t k=(uint64_t(prev5_)<<48)|(uint64_t(prev4_)<<40)|(uint64_t(prev3_)<<32)|(uint64_t(prev2_)<<24)|(uint64_t(prev1_)<<16)|node;
            add(get(o5_,k).c,13);
        }
    ...
    void update_bit(uint16_t node,int b){
        ...
        if(have4_){
            uint64_t k=(uint64_t(prev4_)<<40)|(uint64_t(prev3_)<<32)|(uint64_t(prev2_)<<24)|(uint64_t(prev1_)<<16)|node;
            get(o4_,k).c.update(b);
        }
        if(have5_){
            uint64_t k=(uint64_t(prev5_)<<48)|(uint64_t(prev4_)<<40)|(uint64_t(prev3_)<<32)|(uint64_t(prev2_)<<24)|(uint64_t(prev1_)<<16)|node;
            get(o5_,k).c.update(b);
        }
    }
    ...
    void finish_byte(uint8_t c){
        if(hist_.size()>=8){
            uint64_t key=context8_key();uint32_t idx=uint32_t(key)&MMASK;
            MEntry &e=match_[idx];uint32_t p=uint32_t(hist_.size());
            if(e.key!=key){e=MEntry();e.key=key;e.pos[0]=p;e.count=1;}
            else if(!e.count || e.pos[0]!=p){
                for(int j=std::min<int>(3,e.count);j>0;j--)e.pos[j]=e.pos[j-1];
                e.pos[0]=p;if(e.count<4)e.count++;
            }
        }
        int64_t lp=last_[c];
        if(lp>=0){
        ...
        hist_.push_back(c);
        prev5_=prev4_;prev4_=prev3_;prev3_=prev2_;prev2_=prev1_;prev1_=c;
        if(have4_)have5_=true;
        if(have3_)have4_=true;
...
static void encode(const std::string&src,const std::string&dst,uint8_t minmatch){
    ...
    out.write("HC41",4);out.put(1);out.put(char(minmatch));put_u64(out,n);
    ...
    while(pos<n){
        m.start_byte();uint8_t kc=m.match_count();bool has=kc>0;uint64_t L=0;uint8_t best=0;
        if(has){
            for(uint8_t ci=0;ci<kc;ci++){
                uint64_t s=m.match_source(ci),cl=0,lim=std::min<uint64_t>(MAX_MATCH,n-pos);
                while(cl<lim && data[s+cl]==data[pos+cl])cl++;
                if(cl>L){L=cl;best=ci;}
            }
        }
        ...
        if(use){
            for(uint8_t j=0;j+1<kc;j++){int stop=(best==j);ac.bit(stop,1,1);if(stop)break;}
...
static void decode(const std::string&src,const std::string&dst){
    ...
    char magic[4];in.read(magic,4);if(in.gcount()!=4||std::string(magic,4)!="HC41")throw std::runtime_error("magic");
    ...
    uint64_t pos=0,matches=0,match_bytes=0,literals=0;
    while(pos<n){
        m.start_byte();uint8_t kc=m.match_count();bool has=kc>0,use=false;
        if(has){uint32_t f0=flag.n0,f1=flag.n1;use=ac.bit(f0,f1);rescale_flag(flag,use?1:0);}
        if(use){
            uint8_t best=kc-1;
            for(uint8_t j=0;j+1<kc;j++){if(ac.bit(1,1)){best=j;break;}}
            uint64_t L=gamma_dec(ac)+minmatch-1;if(!L||pos+L>n)throw std::runtime_error("match length");
            uint64_t s=m.match_source(best);matches++;match_bytes+=L;
...
int main(int argc,char**argv){
    try{
        if(argc<4||argc>5){std::cerr<<"usage: hmc_domino_v4 e input output.hcd [minmatch=8] | d input.hcd output\n";return 2;}
