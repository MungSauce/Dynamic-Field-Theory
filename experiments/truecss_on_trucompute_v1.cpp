...
constexpr bool directional(State s){ return s==State::NEG || s==State::POS; }
...
inline uint8_t relative(State a, State ref){
    if(!directional(a) || !directional(ref))
        throw std::runtime_error("TruCompute relative() requires directional live states");
    return a==ref ? 0 : 1;
}
inline State from_orientation(uint8_t bit){
    return bit ? State::NEG : State::POS;
}
...
struct RelativeConstraintSolver{
    ...
    trucompute::PackedGraph freeze(){
        ...
        for(uint32_t edge=0;edge<EDGE_COUNT;edge++){
            if(!active[edge]){
                graph.set(edge,trucompute::State::DEAD_STOP);
                continue;
            }
            auto f=find(edge);
...
static uint8_t resolve_bit(const trucompute::PackedGraph& graph,uint64_t t,uint64_t ctx,int bit){
    ...
    if(composite!=State::LIVE_ZERO)
        throw std::runtime_error("routed pair is not TruCompute LIVE_ZERO");
    if(net(composite)!=0 || activity(composite)!=2)
        throw std::runtime_error("TruCompute live-zero invariant failed");
    if(!directional(ref))
        throw std::runtime_error("TruCompute reference is DEAD_STOP/non-directional");
...
static int train(const std::string&src,const std::string&state,uint64_t limit){
    ...
    for(uint32_t edge=0;edge<EDGE_COUNT;edge++){
        switch(graph.get(edge)){
            case trucompute::State::DEAD_STOP:dead++;break;
            case trucompute::State::NEG:neg++;break;
            case trucompute::State::POS:pos++;break;
            case trucompute::State::LIVE_ZERO:live++;break;
        }
    }
...
static int selftest(){
    using namespace trucompute;
    if(combine(State::NEG,State::POS)!=State::LIVE_ZERO)return 1;
    if(net(State::LIVE_ZERO)!=0)return 2;
    if(activity(State::LIVE_ZERO)!=2)return 3;
    if(!dead(State::DEAD_STOP))return 4;
