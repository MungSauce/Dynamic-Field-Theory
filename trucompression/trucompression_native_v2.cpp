#include "../trucompute/trucompute_runtime_v2.hpp"
...
struct Machine {
    ...
    State query_bit(uint64_t page, uint32_t local_pos, uint8_t bit) const {
        ...
        if (trucompute::dead(sa) || trucompute::dead(sb))
    ...
    bool decoded_byte(uint64_t page, uint32_t local_pos, uint8_t &out) const {
        ...
        for (uint8_t bit = 0; bit < 8; ++bit) {
            ...
            if (s == State::LIVE_ZERO || s == State::DEAD_STOP)
                return false;
            if (s == State::POS)
                out |= uint8_t(1u << bit);
        }
    ...
    State press_character(uint64_t page, uint32_t local_pos, uint16_t button) const {
        ...
        if (!decoded_byte(page, local_pos, decoded))
            return State::LIVE_ZERO; // addressed but unresolved = stateless
...
static void save_machine(const Machine &m, const std::string &path) {
    ...
    h.route_id = 0x545255434F4D5032ULL;
...
static Machine load_machine(const std::string &path) {
    ...
    if (std::memcmp(h.magic, "TRUCOMP2", 8)
        || h.version != VERSION
        || h.node_count != NODE_COUNT
        || h.page_size != PAGE_SIZE
        || h.button_count != BUTTON_COUNT
...
static int selftest() {
    ...
    if (e.observe(false) != State::DEAD_STOP) return 2;
    if (e.observe(true) != State::POS) return 3;
    if (e.observe(true, true) != State::NEG) return 4;
    if (settle({+1, -1}) != State::LIVE_ZERO) return 5;
    if (!stateless(settle({+1, -1}))) return 6;
    if (net(settle({+1, -1})) != 0) return 7;

    std::cout
        << "TRUCOMPUTE_V2_CONFORMANCE=PASS\n"
        << "TRUCOMPRESSION_GATED_MACHINE=PASS\n"
        << "physical_nodes=" << NODE_COUNT << "\n"
        << "structurally_live_nodes=" << NODE_COUNT << "\n"
        << "inactive_signal=DEAD_STOP_0\n"
        << "active_sum_zero=STATELESS\n"
        << "retained_polarity_bits=" << NODE_COUNT << "\n"
        << "button_order=0..255\n";
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        std::cerr
            << "usage:\n"
            << "  trucompression_v2 selftest\n"
            << "  trucompression_v2 imprint SOURCE ARTIFACT\n"
            << "  trucompression_v2 replay ARTIFACT OUTPUT [--strict-buttons]\n";
        return 64;
