...
static void save(const Machine &m, const std::string &path) {
    ...
    h.transfer_id = 0x545255484D433035ULL;
...
static Machine load(const std::string &path) {
    ...
    if (std::memcmp(h.magic, "TRUHMC05", 8)
        || h.version != VERSION
        || h.node_count != NODE_COUNT
        || h.page_size != PAGE_SIZE
        || h.page_buttons != PAGE_BUTTONS
...
static int selftest(){
    ...
    std::cout
        ...
        <<"TRUCOMPRESSION_HMC_SELECTOR=PASS\n"
        <<"physical_nodes="<<NODE_COUNT<<"\n"
        <<"page_buttons="<<PAGE_BUTTONS<<"\n"
        <<"character_buttons="<<CHAR_BUTTONS<<"\n"
        <<"control_buttons="<<CONTROL_BUTTONS<<"\n"
...
int main(int argc,char**argv){
    using namespace trucompression_hmc;
    try{
        ...
        std::cerr
            <<"usage:\n"
            <<"  trucompression_hmc_v5 selftest\n"
            <<"  trucompression_hmc_v5 blank ARTIFACT\n"
            <<"  trucompression_hmc_v5 imprint SOURCE ARTIFACT\n"
            <<"  trucompression_hmc_v5 replay ARTIFACT OUTPUT [--strict-buttons]\n"
            <<"  trucompression_hmc_v5 synth LENGTH OUTPUT\n";
        return 64;
