#include "../trucompute/trucompute_runtime_v1.hpp"
...
static constexpr uint16_t BUTTON_COUNT = 256; // fixed generic order: 0..255
...
static Route route(uint64_t page, uint32_t local_pos, uint8_t bit) {
    ...
    uint8_t modifier = uint8_t(mix64(base ^ 0xA4093822299F31D0ULL) & 1ULL);
    return {a, b, modifier};
}

// Temporary imprint-time solver only. It is destroyed before replay.
struct ParityDSU {
    std::vector<uint32_t> parent;
    std::vector<uint8_t> rank;
    std::vector<uint8_t> parity; // stored(x) xor stored(parent[x])
    ...
    bool constrain(uint32_t a, uint32_t b, uint8_t wanted_relation) {
        auto [ra, pa] = find(a);
        auto [rb, pb] = find(b);

        if (ra == rb) return uint8_t(pa ^ pb) == wanted_relation;
        ...
        parent[rb] = ra;
        parity[rb] = uint8_t(pa ^ pb ^ wanted_relation);
...
struct Machine {
    ...
    // Page voltage changes the expressed polarity without creating page storage.
    uint8_t effective(uint64_t page, uint32_t node) const {
        return uint8_t(stored(node) ^ page_phase(page, node));
    }

    State query_bit(uint64_t page, uint32_t local_pos, uint8_t bit) const {
        const auto r = route(page, local_pos, bit);
        const uint8_t result =
            uint8_t(effective(page, r.a) ^ effective(page, r.b) ^ r.modifier);
        return result ? State::POS : State::NEG;
    }

    uint8_t decoded_byte(uint64_t page, uint32_t local_pos) const {
        uint8_t out = 0;
        for (uint8_t bit = 0; bit < 8; ++bit) {
            if (query_bit(page, local_pos, bit) == State::POS)
                out |= uint8_t(1u << bit);
        }
        return out;
    }

    // Character voltage asks exactly one structural question:
    // "Is the highlighted byte present at this position?"
    State press_character(uint64_t page, uint32_t local_pos, uint16_t button) const {
        if (button >= BUTTON_COUNT) return State::DEAD_STOP;
        return decoded_byte(page, local_pos) == uint8_t(button)
            ? State::POS
            : State::NEG;
    }
};
...
static void save_machine(const Machine &m, const std::string &path) {
    ...
    h.route_id = 0x545255434F4D5031ULL;
...
static Machine load_machine(const std::string &path) {
    ...
    if (std::memcmp(h.magic, "TRUCOMP1", 8)
        || h.version != VERSION
        || h.node_count != NODE_COUNT
        || h.page_size != PAGE_SIZE
        || h.button_count != BUTTON_COUNT
...
static int imprint(const std::string &source_path, const std::string &artifact_path) {
    ...
    // WRITE TIME ONLY: source is present here.
    for (uint64_t t = 0; t < source.size(); ++t) {
        ...
        for (uint8_t bit = 0; bit < 8; ++bit) {
            ...
            // effective(a,page) xor effective(b,page) xor modifier = target
            // therefore stored(a) xor stored(b) must equal:
    ...
    // Freeze verification while source is still present.
    // Only the actual replay law is used; solver state is not consulted.
    for (uint64_t t = 0; t < source.size(); ++t) {
        const uint64_t page = t / PAGE_SIZE;
        const uint32_t local = uint32_t(t % PAGE_SIZE);
        if (machine.decoded_byte(page, local) != source[t]) {
...
static int replay(const std::string &artifact_path,
    ...
    // READ TIME ONLY: no source path exists in this command.
    ...
    for (uint64_t page = 0; page < pages; ++page) {
        ...
        if (strict_buttons) {
            ...
            // Fixed, source-independent order of character-button pushes.
            for (uint16_t button = 0; button < BUTTON_COUNT; ++button) {
                for (uint32_t local = 0; local < page_bytes; ++local) {
                    if (machine.press_character(page, local, button) == State::POS) {
                        if (resolved[local] != -1) {
                            std::cout
                                << "status=AMBIGUOUS\n"
                                << "page=" << page << "\n"
            ...
            // Equivalent fast path. Strict-button mode is the conformance path.
            for (uint32_t local = 0; local < page_bytes; ++local) {
                const uint8_t byte = machine.decoded_byte(page, local);
                out.write(reinterpret_cast<const char *>(&byte), 1);
            }
        }
...
static int selftest() {
    ...
    const auto live = combine(encode_binary(0), encode_binary(1));

    if (live != State::LIVE_ZERO) return 1;
    if (net(live) != 0) return 2;
    if (activity(live) != 2) return 3;
    if (State::DEAD_STOP == State::LIVE_ZERO) return 4;
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        std::cerr
            ...
            << "  trucompression imprint SOURCE ARTIFACT\n"
            << "  trucompression replay ARTIFACT OUTPUT [--strict-buttons]\n";
        return 64;
