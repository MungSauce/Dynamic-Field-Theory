...
from signal_scrambler import scramble_samples
    ...
    try:
        with open(tmp,"wb") as out:
            for t0 in range(0,frames,BLOCK_FRAMES):
                ...
                # Audio/radio-adjacent synthesis: two quadrature note coordinates,
                # Walsh spreading per instrument, then PRN chip scrambling.
                I=fwht_rows(F,p);Q=fwht_rows(R,p)
                composite=(I+p*Q).astype(np.uint16,copy=False).reshape(-1)
                recorded=scramble_samples(composite,samples,signal_base)
                raw=pack_samples(recorded,signal_base)
                out.write(raw);ph.update(raw);pbytes+=len(raw);samples+=len(recorded)
    finally:
        del src
