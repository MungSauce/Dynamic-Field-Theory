...
from signal_scrambler import descramble_samples
...
def listen(msf,output):
    ...
    bounds=page_bounds(n,N);alphabet_arr=np.frombuffer(alphabet,dtype=np.uint8)
    out=np.memmap(output,dtype=np.uint8,mode="w+",shape=(n,))
    bf=h["block_frames"];sample_index=0
    with open(msf,"rb") as f:
        f.seek(payload_offset)
        for t0 in range(0,frames,bf):
            t1=min(frames,t0+bf);count=(t1-t0)*N
            raw=f.read(packed_bytes_for_samples(count))
            if len(raw)!=packed_bytes_for_samples(count):raise ValueError("short signal payload")
            recorded=unpack_samples(raw,signal_base,count)
            composite=descramble_samples(recorded,sample_index,signal_base).astype(np.int64).reshape(t1-t0,N)
            sample_index+=count
            # Audience: despread/matched-filter the recording using the generic
            # instrument signature bank, then interpret the shared note lexicon.
            I=composite%p;Q=composite//p
    ...
    return {"recovered_bytes":n,"recovered_sha256":got.hex(),"exact_reconstruction":True,
            "decoder_input":"noise-like recorded signal only",
            "audience_method":"PRN descramble + Walsh matched filtering + shared lexicon",
            "composer_state_used":False}
