...
sys.path.insert(0,str(ROOT/"msf_recording_v31"))
...
def main():
    ...
    n=src.stat().st_size; ss=sha256_file(src);msf=Path("enwik100m_v31_N128.msf")
    t=time.perf_counter();cr=compose(src,msf,N);t1=time.perf_counter()
    src.unlink()
    enc=ROOT/"msf_recording_v31"/"orchestra_encoder.py"
    if enc.exists():enc.unlink()
    aud=Path("audience_v31")
    if aud.exists():shutil.rmtree(aud)
    aud.mkdir();shutil.copy2(msf,aud/"recording.msf")
    for name in ("audience_decoder.py","signal_field.py","signal_packing.py","msf_format.py"):
        shutil.copy2(ROOT/"msf_recording_v31"/name,aud/name)
    ...
    result={
      "architecture":"MSF recording v3.1 — literal composite signal with signal-sample packing",
      "source":"first 100,000,000 bytes of canonical enwik9 (enwik8)",
      ...
      "decoder_input":"packed literal recorded signal only",
    ...
    Path("msf_recording_v31_100mb_report.json").write_text(json.dumps(result,indent=2)+"\n")
